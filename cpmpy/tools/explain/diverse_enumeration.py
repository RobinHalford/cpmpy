...
def marco_diverse_Min(soft, hard=[], solver="exact", map_solver="exact", return_mus=True, return_mcs=False, do_solution_hint=True, time_limit=None):
    ...
    while True:
        ...
        if sat_result is True:
            ...
            # (a blocked similar MSS will stimulate more diverse MUS)
...
def marco_diverse_shrink(soft, hard=[], solver="exact", map_solver="exact", return_mus=True, return_mcs=False, time_limit=None):
    """
        A modified version of MARCO where the Grow and Shrink procedures' selection order is favoring diversity.
...
def marco_diverse_shrink_noCore(soft, hard=[], solver="exact", map_solver="exact", return_mus=True, return_mcs=False, time_limit=None):
    """
        A modified version of MARCO where the Grow and Shrink procedures' selection order is favoring diversity.
    ...
    while True:
        ...
        if sat_result is True:
            ...
            # start from seed; skip clause-set refinement
            mus = set(seed)
            for c in sorted(mus, key=seenmap.get, reverse=True):
                if c not in mus: # already removed
                    continue
                mus.remove(c)
                shrink_result = timed_solve(s, deadline, assumptions=list(mus))
                if shrink_result is None:
                    yield "TIMEOUT", None, time.monotonic() - start_time
                    return
                if shrink_result is True:
            ...
            map_solver += ~cp.all(mus) # block in map solver
            ...
            if return_mus:
                yield "MUS", [dmap[a] for a in mus], time.monotonic() - start_time

        map_solver.solution_hint(assump, [1]*len(assump)) # we want large subsets, more likely to be a MUS


def marco_diverse_solhint(soft, hard=[], solver="exact", map_solver="exact", return_mus=True, return_mcs=False, time_limit=None):
    """
        A modified version of MARCO where the Grow and Shrink procedures' selection order is favoring diversity, 
